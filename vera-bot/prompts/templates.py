...
"""
    ...
    kind = trigger.get('kind', '')
    kind_instruction = {
        'research_digest':      'Lead with the specific research finding. Quote the exact numbers from the digest title (percentages, trial_n, patient_segment) — do not paraphrase them. End with exactly the citation string from "formatted_citation" in Extracted facts — copy it verbatim, do not rephrase. Do NOT open with CTR or performance metrics.',
        'regulation_change':    'Lead with the compliance deadline and what specifically changed. Be precise about dates and requirements. Do NOT reference CTR, peer metrics, or performance data — this is a compliance message only. Apply effort_externalization: tell them you have already prepared a compliance checklist or audit draft for their setup. If "formatted_citation" is present in Extracted facts, cite it verbatim at the end. End with a yes/stop CTA asking if they want you to run the audit now.',
        ...
        'supply_alert':             'Lead with the specific molecule and affected batch numbers from the payload. Name the manufacturer. State the pre-computed patient count from "supply_alert_patient_estimate" in Extracted facts — use that exact number (e.g. "X of your chronic-Rx customers may be on this"). State the immediate action the merchant must take (check stock, quarantine affected batches, notify patients on this molecule). Urgency is high — no soft language. yes/stop CTA.',
